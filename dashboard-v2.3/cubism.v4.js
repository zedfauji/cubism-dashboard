...
cubism.context = function() {
  var context = new cubism_context,
      ...
      serverDelay = 5e3,
      clientDelay = 5e3,
      event = d3.dispatch("prepare", "beforechange", "change", "focus","blur"),
 ...
 context.blur = function(i) {
  event.blur.call(context, blur = i);
  return context;
 };
  ...
  context.on = function(type, listener) {
    ...
    if (listener != null) {
      ...
      if (/^blur(\.|$)/.test(type)) listener.call(context, blur);
    }
...
cubism_contextPrototype.metric = function(request, name) {
  ...
  //--Paritosh 28-10-2013---
  metric.find = function(pattern,_host,_HTMLTAG_,_INDEX_, callback) {
    d3.json(_host + "/metrics/find?format=completer"
        + "&query=" + encodeURIComponent(pattern), function(result) {
      if (!result) return callback(new Error("unable to find metrics"));
      callback(null, result.metrics.map(function(d) { return d.path; }),_HTMLTAG_,_INDEX_);
    });
  };
...
cubism_contextPrototype.horizon = function() {
  ...
  function horizon(selection) {
    ...
    selection
        ...
        .on("mouseout.horizon", function() { context.focus(null); context.blur(); });
	...
	//---need to review the below function---
      function blur(i) { }
